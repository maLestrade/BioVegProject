...
public class PrimerBlastHtmlParser {
  ...
  private SeqFeatureI processData(String line, int primerNum, int offset, boolean plus)
  {
    ...
    // MODIFY BY PIDUPUIS
    ((SeqFeature) feat).setTm(Double.parseDouble(tokens[7]));
    ((SeqFeature) feat).setGc(Double.parseDouble(tokens[8]));
    ((SeqFeature) feat).setSelfCompAny(Double.parseDouble(tokens[9]));
    ((SeqFeature) feat).setSelfCompEnd(Double.parseDouble(tokens[10]));
    //
